# FLASH Bridge - MVP Demo Docker Image
# Enterprise-grade complete user experience demonstration

FROM node:18-alpine

# Install system dependencies including browser for visual demo
...
# Expose ports for comprehensive demo access
EXPOSE 3000 3001

# Enhanced health check with enterprise monitoring
HEALTHCHECK --interval=30s --timeout=10s --start-period=60s --retries=3 \
    CMD curl -f http://localhost:3001/health || exit 1

# Enterprise demo command - complete user experience validation
CMD ["node", "demo-mvp.js"]
